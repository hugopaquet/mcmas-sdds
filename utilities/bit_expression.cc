...
bit_expression::encode_sdd_true(SddManager* manager, vector<SddNode*> * v)
{
  ...
  if (op == 9) {    // and "&"
    ...
    res = sdd_disjoin(sdd_conjoin(lhs1, rhs2, manager), sdd_disjoin(lhs2, rhs1, manager), manager);
  }
  return res;
}
...
bit_expression::encode_sdd_false(SddManager* manager, vector<SddNode*> * v)
{
  ...
  if (op == 9) {    // and "&"
    ...
    SddNode* lhs1 = operands[0]->encode_sdd_false(manager, v);
    SddNode* rhs1 = operands[1]->encode_sdd_false(manager, v);
    SddNode* lhs2 = operands[0]->encode_sdd_true(manager, v);
    SddNode* rhs2 = operands[1]->encode_sdd_true(manager, v);
    res = sdd_disjoin(sdd_conjoin(lhs1, rhs2, manager), sdd_disjoin(lhs2, rhs1, manager), manager);
  }
  return res;
}
