		    ...
		    for (unsigned int i = 0; i < agents->size(); i++) {
					...
					if (vec_obsvars != NULL)
						for (unsigned int j = 0; j < vec_obsvars->size(); j++) {
							...
							Cudd_MakeTreeNode(bddmgr->getManager(), a1, a2 - a1 + 1, MTR_DEFAULT);
							Cudd_MakeTreeNode(bddmgr->getManager(), states_count + a1, a2 - a1 + 1,	MTR_DEFAULT);
	  				}			
					if (vec_vars != NULL)
					...
					if (options["bddgroup"] == 3) {
						int l = 0;
						for (unsigned int i = 0; i < agents->size(); i++) {
							Cudd_MakeTreeNode(bddmgr->getManager(),	states_count * 2 + l,	(*agents)[i]->actions_BDD_length(), MTR_DEFAULT);
