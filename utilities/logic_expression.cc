...
logic_expression::encode_sdd(SddManager* manager, struct parameters* params) { 
  ...
  int rhstype = operands[1]->get_type();
    ...
    if (value->get_value())
      node = sdd_conjoin(node, (*params->variable_sdds)[begin], manager);
    else
      node = sdd_conjoin(node, sdd_negate((*params->variable_sdds)[begin], manager), manager);
		sdd_ref(node, manager);
    if (op == 1)
			encoding = sdd_conjoin(encoding, sdd_negate(node, manager), manager);
    else
			encoding = sdd_conjoin(encoding, node, manager);
		sdd_ref(encoding, manager);
		sdd_deref(node, manager);


  } else if ((lhstype >= 9 && lhstype <= 12) || (rhstype >= 9 && rhstype <= 12)) {  // bit expression
			cout << "We have a bit expression: " << to_string() << endl;
		if (lhstype == 1) { // lhs is a constant
			bool_value *value = (bool_value *) operands[0];
			if(value->get_value())
				node = ((bit_expression *) operands[1])->encode_sdd_true(manager, params->variable_sdds);
			else
				node = ((bit_expression *) operands[1])->encode_sdd_false(manager, params->variable_sdds);
			sdd_ref(node, manager);
		} else if (rhstype == 1) { // rhs is a constant
			bool_value *value = (bool_value *) operands[1];
			if(value->get_value())
				node = ((bit_expression *) operands[0])->encode_sdd_true(manager, params->variable_sdds);
			else
				node = ((bit_expression *) operands[0])->encode_sdd_false(manager, params->variable_sdds);
			sdd_ref(node, manager);
		} else if (lhstype == 0) { // lhs is a variable
			variable *var = (variable *) operands[0];
			basictype *var_type = var->get_var_type();
			begin = var_type->get_index_begin();
			SddNode* node1 = sdd_conjoin((*params->variable_sdds)[begin] , ((bit_expression *) operands[1])->encode_sdd_true(manager, params->variable_sdds), manager);
			sdd_ref(node1, manager);
			SddNode* node2 = sdd_conjoin(sdd_negate((*params->variable_sdds)[begin], manager), ((bit_expression *) operands[1])->encode_sdd_false(manager, params->variable_sdds), manager);
			sdd_ref(node2, manager);
			node = sdd_disjoin(node1,	node2, manager);
			sdd_ref(node, manager);
			...
			basictype *var_type = var->get_var_type();
			begin = var_type->get_index_begin();
			SddNode* node1 = sdd_conjoin((*params->variable_sdds)[begin] , ((bit_expression *) operands[0])->encode_sdd_true(manager, params->variable_sdds), manager);
			sdd_ref(node1, manager);
			SddNode* node2 = sdd_conjoin(sdd_negate((*params->variable_sdds)[begin], manager), ((bit_expression *) operands[0])->encode_sdd_false(manager, params->variable_sdds), manager);
			sdd_ref(node2, manager);
			node = sdd_disjoin(node1,	node2, manager);
			sdd_ref(node, manager);
			sdd_deref(node1, manager);
			...
			SddNode* node1 = sdd_conjoin(((bit_expression *) operands[0])->encode_sdd_true(manager, params->variable_sdds), ((bit_expression *) operands[1])->encode_sdd_true(manager, params->variable_sdds), manager);
			sdd_ref(node1, manager);
			SddNode* node2 = sdd_conjoin(((bit_expression *) operands[0])->encode_sdd_false(manager, params->variable_sdds), ((bit_expression *) operands[1])->encode_sdd_false(manager, params->variable_sdds), manager);
			sdd_ref(node2, manager);
			node = sdd_disjoin(node1,	node2, manager);
			sdd_ref(node, manager);
			sdd_deref(node1, manager);
    ...
    if (op == 1)
      encoding = sdd_conjoin(tmp = encoding, sdd_negate(node, manager), manager);
    else
      encoding = sdd_conjoin(tmp = encoding, node, manager);
		sdd_ref(encoding, manager);
		sdd_deref(node, manager);
		sdd_deref(tmp, manager);
		cout << "for the expression " << to_string() << " we are saving the file." << endl;
		sdd_save_as_dot("logic.dot", encoding);
