...
compute_reach(SddNode* in_st, SddManager* manager, struct parameters * params, vector<SddNode*>* transition_relation_sdds) {
  ...
  SddNode* reach = in_st;
  SddNode* next1 = in_st;
	SddNode* q1 = sdd_manager_false(manager);
  ...
  while (true) {
      struct timeb tmb;
    ftime(&tmb);
		 cout << "reach has count " << sdd_count(reach) << endl;
...
//		sdd_vtree_save_as_dot("t3.dot", sdd_manager_vtree(manager));
    ...
    cout.flush();
    	   ftime(&tmb2);
		q1 = sdd_disjoin(next1, reach, manager);
		sdd_ref(q1, manager);

    if (q1 == reach)
      break;
    else {
			sdd_deref(reach, manager);
		  reach = q1;
			sdd_ref(reach, manager);
			sdd_deref(q1, manager);
		}
//	if(options["dao"])
	//  	sdd_manager_auto_gc_and_minimize_on(manager); 
				ftime(&tmb3);
...
create_vtree() {
	...
	switch(options["vtree"]) {
		case 1:
			var_order = get_var_order(options["ordering"], agents);
        ...
        struct vtree_node* leaf = new struct vtree_node;
	      ...
	      leaves.push_back(leaf);
			...
			for(int i = 0; i < agents->size(); i++) {
				vector<struct vtree_node*> vars;
				vector<struct vtree_node*> actions;
		    for (int j = (*agents)[i]->get_action_index_begin(); 
		                          j <= (*agents)[i]->get_action_index_end(); j++) 
			    actions.push_back(leaves[2*states_count + j]);
		    for (int j = (*agents)[i]->get_var_index_begin(); 
		                          j <= (*agents)[i]->get_var_index_end(); j++) {
			    vars.push_back(leaves[j]);
			    vars.push_back(leaves[states_count + j]);
			  }
				vector<struct vtree_node*> var_subtrees;	
				var_subtrees.push_back(make_balanced_vtree(actions));
				for(int k = 0; k < vars.size(); k++) {
					struct vtree_node * pair_sub =	new vtree_node;
					pair_sub->left = vars[k];
					pair_sub->right = vars[k+1];				
					pair_sub->size = 3;
					pair_sub->isleaf = false;
					pair_sub->id = vtree_node_id_counter++;
					k++;
					var_subtrees.push_back(pair_sub);
				}

				struct vtree_node * agent_root = var_subtrees[0];
				for(int k = 1; k < var_subtrees.size(); k++) {
					struct vtree_node* agent_new_root = new vtree_node;
        	agent_new_root->left = var_subtrees[k];
        	agent_new_root->right = agent_root;
        	agent_new_root->id = vtree_node_id_counter++;
        	agent_new_root->isleaf = false;
        	agent_new_root->size = var_subtrees[k]->size + agent_root->size + 1;
        	agent_root = agent_new_root;
				}
				subtrees.push_back(agent_root);
			}
  ...
  vector<SddNode*>* transition_relation_sdds = new vector<SddNode*>;
	...
	for (unsigned int i = 0; i < agents->size(); i++) {
		...
		cout << " - Done. " << endl;
			...
			cout << "the count of  protocol is " << sdd_count(protocol_sdd) << endl;
...
//	cout << "The manager has count " << sdd_manager_count(manager)  << "(dead " << sdd_manager_live_count(manager) <<  ", live " << sdd_manager_dead_count(manager) << ")" << endl;
  ...
  if (!is_fairness->empty()) {
	...
	cout << "there are " << is_formulae->size() << " formulae to check" << endl;
