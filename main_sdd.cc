...
*/
  ...
  vector<SddNode*>* transition_relation_sdds = new vector<SddNode*>;
	...
	sdd_save_as_dot("sdd.dot", reachable_state_sdd);	

	// check for deadlocks (EX(True))
	// check for arithmetic overflow

	// check formulae
   for (unsigned int i = 0; i < is_formulae->size(); i++) {
			cout << "checking formula:  " << ((*is_formulae)[i]).to_string() << endl;
      SddNode* result = ((*is_formulae)[i]).check_formula(manager, params);
			
			bool satisfaction = result == params->reach;
			cout << "formula " << i << " is " << (satisfaction ? "true" : "false") << endl;
	} 
